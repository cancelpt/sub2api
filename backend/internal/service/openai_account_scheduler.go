...
func (s *defaultOpenAIAccountScheduler) buildOpenAICandidateScores(
	...
	if len(accounts) == 0 {
		return nil, 0, noAvailableOpenAISelectionError(req.RequestedModel, false)
	}
	...
	if req.RequireCompact {
		...
		if len(candidates) == 0 {
			if compactBlocked {
				return nil, 0, ErrNoAvailableCompactAccounts
			}
			return nil, 0, noAvailableOpenAISelectionError(req.RequestedModel, true)
		}
	}
...
func (s *defaultOpenAIAccountScheduler) selectByStrictPriorityFallback(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		result, acquireErr := s.service.tryAcquireAccountSlot(ctx, fresh.ID, fresh.Concurrency)
		if acquireErr != nil {
			return nil, candidateCount, 1, loadSkew, acquireErr
		}
		if result != nil && result.Acquired {
			if req.SessionHash != "" {
				_ = s.service.BindStickySession(ctx, req.GroupID, req.SessionHash, fresh.ID)
	...
	return nil, candidateCount, 1, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, compactBlocked)
}
...
func (s *OpenAIGatewayService) getOpenAIAccountScheduler(ctx context.Context) OpenAIAccountScheduler {
	...
	shouldEnable := s.isOpenAIAdvancedSchedulerEnabled(ctx)
	if !shouldEnable {
		if s.settingService != nil {
			shouldEnable = true
		} else if s.cfg != nil && strings.TrimSpace(s.cfg.Gateway.OpenAIWS.SchedulerMode) != "" {
			shouldEnable = true
		}
	}
	if !shouldEnable {
